...
| 2026-08-24 | **핀 폭 절반(15→7.5nm) 검증 (남다연)** — `baseline/verification_finwidth_half_G60_G70.xlsx`(Ge 60/70% × FR 0/10/20/30/35). ⚠ 파일에 파라미터 열(Ge/FR/W)이 없어 앞 5행=Ge60, 뒤 5행=Ge70 으로 **추정 매핑** — 남다연 확인 필요 | ① **가설 입증**: FR=0 에서 SSlin 77.4→66.3(Ge60), 76.6→65.1(Ge70), DIBL 약 60% 감소 → 기준 구조의 SSlin 76~80 은 소자/모델 결함이 아니라 fin 폭 대 게이트 길이 비율(60%)에서 오는 구조적 결과임이 확정. ② **예상 밖**: 응력·STE 도 전 조건 +9~11% 상승(fin 이 얇을수록 탄성 완화 감소) → fin 폭이 FR 에 이은 세 번째 기하학적 전달 인자. ③ **★ 역전**: FR=30 부터 얇은 fin 이 오히려 불리, FR=35 에서 SSlin 2배 악화·Ioff 약 60배·\|VtiSat\| 0.07V 붕괴 — 게이트 도달 범위 밖 누설 경로는 fin 폭으로 통제 불가. → fin 폭이 줄수록 FR 상한이 더 엄격해짐(결론 5 강화) | — (PR 생략, 직접 반영) |
