...
예: x=0.3(Ge 30%) → Tc ≈ 52nm. (다른 문헌의 "Ge 40%→Tc≈50nm"와 대략 정합 — 단 근사식이라 팀이 원 논문 그래프와 직접 대조 권장)
...
1. People-Bean 기준선 (보수적, 평면 가정)
2. Luryi-Suhir 보정선 (fin 구속 반영, fin 폭은 baseline 예제의 문서 스펙에서 확정)

두 선 사이의 영역이 "평면 기준으론 위험해 보이지만 실제 fin 구조에서는 안전한 영역" — 이게 우리 프로젝트만의 정량적 주장으로 활용 가능 (2-3절 독창성 근거와 연결).
...
5. People-Bean 근사식(Tc≈1.23x⁻³·⁰⁸)을 원 논문 그래프와 대조해 오차 확인
