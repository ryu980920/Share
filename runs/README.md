...
| 컬럼 | 뜻 |
|---|---|
| `run_id` | 격자점 이름 (제안 표기 — 팀 확정 필요, README.md 참고). `G{Ge조성%}_F{FR_nm}` → `G50_F10` = Ge 50%, FR(리세스 깊이) 10nm |
| `stress_GPa` | 채널 인접 지점의 응력 [GPa]. 정확한 추출 정의는 `analysis/config.yaml` 한 곳에서만 정한다 — **손으로 다르게 뽑지 말 것** |
| `ste` (선택) | Stress Transfer Efficiency. **정규화 방법이 아직 팀 확정 전이라 필수 컬럼이 아니다** — 있으면 `build.py` 가 통과시키기만 하고 계산하지는 않는다 |
...
SWB 의 파라미터 이름이 그대로 격자 좌표가 된다. `GeMoleFraction = 0.50`, `FR_nm = 10` 이면 → `G50_F10` 격자점으로 인식된다.

이름을 바꾸기 어려우면 `analysis/config.yaml` 의 `swb.x_param` / `y_param` 을 실제 이름에 맞추면 된다. `stress_GPa` 에 대응하는 SWB 컬럼 이름도 같은 파일의 `swb.map` 에서 맞춘다 (Phase 0 에서 실제 SWB 출력을 보고 확정 — 지금은 TODO).
...
```bash
cp -r runs/attachments/_예시 runs/attachments/G50_F10
```
...
```bash
git add runs/attachments/G50_F10
git commit -m "G50_F10 첨부물"
git push
```
...
| 격자에 안 올라감 | `run_id` 형식이 다름. `G50F10` ❌ → `G50_F10` ⭕ |
| 대시보드가 안 바뀜 | push 후 Actions 가 끝나기까지 1~2분 걸린다 |
| 첨부물 점이 안 뜸 | 폴더 이름이 `run_id` 와 정확히 일치해야 한다 (`G50_F10`, 대소문자·언더바 포함) |
